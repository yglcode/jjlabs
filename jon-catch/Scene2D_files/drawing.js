 ...
 //       this.enableBallDrag()
    ...
    bounce(){
        for (let i=1; i<this.balls.length;i++){
            ...
            if(hypo <= this.main.radius+this.balls[i].radius){
                this.balls[i].dx = dx/20
                this.balls[i].dy = dy/20
                this.balls[i].start()
            }
        }
    }
    ...
    enableHook(){
        this.on("mousedown", e => {
            ...
            this.on("mousemove", e => {
              ...
              let diff = {
                ...
                for(let i=0;i<this.hooked.length;i++){
                    let diff1 = {
                        x: diff.x * Math.random()*4/4,
                        y: diff.y * Math.random()*4/4
                    }
                    this.hooked[i].move(diff1.x, diff1.y)
                    this.hooked[i].connect.moveEnd(0, diff)
                    this.hooked[i].connect.moveEnd(1, diff1)
                    }
                for (let i = 0;i<this.balls.length;i++){
                    let f = Math.abs(pos.x-this.balls[i].position.x)
                    let g = Math.abs(pos.y-this.balls[i].position.y)
                if (Math.sqrt(f*f+g*g)<=50 && this.balls[i].connect === undefined){
                    this.balls[i].stop()
                    this.hooked.push(this.balls[i])
                    this.balls[i].connect = new Line(this,new Point(pos.x,pos.y),new Point(this.balls[i].position.x,this.balls[i].position.y),1)
                }
            }
                    movepos = pos
                    this.update()
        ...
        this.on("mouseup", e=>{
            ...
            this.hooked.forEach (e => {
                this.remove(e.connect);
                e.connect.selfKill(); 
                delete e.connect})
            delete this.hooked
        })
...
class Line extends Two.Line {
    ...
    selfKill(){
        let i= this.scene2d.lines.indexOf(this)
        this.scene2d.lines.splice(i,1)
    }
    calcBBox() {
